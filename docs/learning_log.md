...
## 2026-07-16：接入天地图国内底图与密钥隔离

### 本次完成

- 使用OpenLayers `XYZ` 数据源接入天地图Web墨卡托矢量底图 `vec_w` 和中文注记 `cva_w`。
- 将底图控制从单个OSM开关改为互斥选择：天地图、OpenStreetMap和简洁底图。
- 配置密钥时默认选择天地图；未配置密钥或显式选择其他底图时保留稳定降级路径。
- 保留项目道路GeoJSON作为最底层容错，避免所有在线瓦片不可用时地图完全失去空间参照。
- 新增 `web/.env.example` 记录环境变量名称，真实密钥只保存在被Git忽略的 `web/.env.local`。
- 在GitHub Pages工作流中使用Actions Secret `VITE_TIANDITU_TOKEN` 向Vite构建过程注入生产密钥。
- 为天地图补充“国家地理信息公共服务平台 天地图”地图服务署名。

### 本次理解

- 天地图矢量底图和中文注记是两个独立瓦片层；只加载 `vec_w` 会有道路和建筑轮廓，但没有中文地名。
- 三种底图使用单选按钮，是因为同一时刻只需要一个主要参考底图；研究区和专题数据仍使用独立复选框。
- Vite中以 `VITE_` 开头的变量会在构建时写入前端代码，因此Actions Secret只能避免密钥进入Git历史，不能让浏览器端密钥真正不可见。
- 浏览器端密钥应依赖域名白名单降低滥用风险；生产密钥只允许正式演示域名，本地域名只用于短期测试。
- 更严格的项目可以分别使用本地开发密钥和生产密钥，两处仍使用相同变量名，但保存不同密钥值。
- 底图属于参考显示层，不改变静态GeoJSON的EPSG:4326存储，也不改变OpenLayers视图使用EPSG:3857实时显示的坐标流程。

### 验证结果

- 使用上海研究区瓦片坐标验证 `vec_w` 和 `cva_w` 均返回HTTP 200及有效PNG。
- 域名白名单仅允许正式演示域名时，本地 `localhost` 和 `127.0.0.1` 请求返回403；临时放行后天地图与中文注记正常显示。
- 天地图、OpenStreetMap和简洁底图三种状态可相互切换，地图署名随当前在线底图改变。
- 390×844视窗下图层抽屉保持300px宽，未产生横向溢出。
- 增加地图容器 `ResizeObserver` 和首次排版后的尺寸同步后，桌面与手机首屏无需缩放即可绘制底图。
- 浏览器控制台没有红色错误，最终生产构建成功转换204个模块。

### 遇到的问题及处理

1. 接入后选择天地图仍显示空白，但OpenStreetMap正常。
   - 排查方法：分别请求矢量和注记瓦片，只输出状态码、内容类型和字节数，再对比不同Referer来源。
   - 原因：密钥域名白名单允许 `cokalex.github.io`，但没有允许本机 `localhost` 和 `127.0.0.1`，浏览器瓦片请求返回403。
   - 处理方法：测试期间临时允许本地域名，验证完成后删除本地白名单，只保留正式演示域名。
2. 自动浏览器截图不能可靠显示天地图跨域Canvas内容。
   - 处理方法：结合瓦片HTTP响应、底图切换状态、地图署名、控制台和用户实际浏览器画面完成验证，不把自动截图作为唯一依据。
3. 刷新页面后在线底图偶尔为空，手动缩放后才出现。
   - 原因：页面首次排版瞬间地图容器尺寸尚未稳定，OpenLayers曾检测到宽或高为0；缩放会触发尺寸重算和重新渲染，所以底图随后恢复。
   - 处理方法：监听地图容器自身尺寸变化，并在连续两个动画帧后、窗口加载后和窗口尺寸变化时调用 `map.updateSize()`。
   - 复验结果：桌面地图容器与视窗均为1280×652，手机端均为390×784；首次加载控制台不再出现尺寸警告。

## 2026-07-16：完成项目最终验收与成果包装

### 本次完成

- 验证本地 `main` 与远程 `origin/main` 完全同步，工作区干净，GitHub仓库为公开状态。
- 扫描全部Git跟踪文件，未发现天地图真实密钥、明文密码或常见API密钥模式。
- 从公网重新验证天地图首屏、静态GeoJSON、设施属性查询、三种底图切换和手机图层抽屉。
- 将README从阶段过程清单重构为面向作品集的成果页，突出在线演示、量化成果、技术架构、WebGIS功能和能力边界。
- 使用已通过公网验收的天地图页面截图作为README成果图，不使用概念稿代替真实成品。
- 增加项目结构、静态与完整服务运行方式、数据限制和AI辅助范围说明。
- 为自行编写的代码增加MIT许可证，同时明确OSM衍生数据仍遵循ODbL。

### 最终验收结果

- 公网页面标题、静态数据模式和默认天地图底图正确，5个专题图层初始均显示。
- 点击医院面成功显示“上海市胸科医院”、设施类型、OSM来源编号和数据来源。
- 天地图与OpenStreetMap切换时图层选择状态和地图署名同步更新。
- 390×844手机首屏地图为390×784，图层面板默认收起；打开后抽屉宽300px且无横向溢出。
- 浏览器控制台无警告或错误，GitHub最新提交检查状态为success。
- 最终整理前仓库共跟踪46个文件；新增成果截图与许可证后预计为48个。整体规模约4.9MB，最大文件仍为两张约2MB的设计概念图，没有异常大文件。

### 本次理解

- HR首先看到的是仓库标题、在线演示、截图和README前几屏，过程记录应放在详细文档而不是阻塞核心成果。
- 作品集README需要同时回答“做了什么、数据有多少、用了什么、怎么验证、如何运行、能力边界是什么”。
- 实际截图与概念设计必须明确区分，不能用概念稿暗示已经实现的功能。
- MIT许可证可以覆盖本人编写的代码，但不能改变OSM数据原有的ODbL许可；代码与数据许可需要分别说明。
- 静态Pages演示与本地PostGIS/GeoServer完整链路均为真实成果，但对外表述时必须明确部署范围，避免把静态演示描述成公网后端。
